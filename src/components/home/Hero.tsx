...
export function Hero() {
  return (
    <section className="relative isolate overflow-hidden bg-[var(--color-bg)]">
      ...
      <div className="mx-auto flex min-h-[100svh] max-w-7xl items-center px-5 py-24 sm:px-6 md:min-h-[820px] md:py-20">
        <div className="max-w-4xl">
          <div className="mb-5 flex flex-wrap gap-2 text-[11px] font-black uppercase tracking-[0.18em] text-[var(--color-accent-strong)] sm:text-xs sm:tracking-[0.22em]">
          ...
          <h1 className="max-w-5xl text-4xl font-black leading-[0.98] tracking-tight text-white drop-shadow-[0_10px_28px_rgba(0,0,0,0.62)] sm:text-6xl lg:text-7xl">
          ...
          <div className="mt-9 flex flex-wrap gap-4">
            <Link
              href="/productos"
              className="w-full rounded-2xl bg-[var(--color-accent)] px-6 py-4 text-center text-sm font-black uppercase tracking-wide text-white shadow-xl transition hover:bg-[var(--color-accent-strong)] sm:w-auto sm:rounded-md sm:px-7"
            >
              Explorar cadena productiva
            </Link>
          </div>

          <div className="mt-8 grid max-w-3xl gap-3 sm:mt-12 sm:grid-cols-3 sm:gap-4">
            {metrics.map(([value, label]) => (
              <div
                key={label}
                className="rounded-2xl border border-[color:var(--color-border)] bg-[var(--color-surface)]/88 p-4 shadow-[var(--shadow-card)] backdrop-blur sm:rounded-xl sm:p-5"
              >
